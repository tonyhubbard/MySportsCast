...
public class MultiCalendarView extends AbstractCalendarView {
	...
	private ViewPager.OnPageChangeListener mOnTitlePageChangeListener;
	...
	private void init() {
		...
		final MultiCalendarAdapter adapter = new MultiCalendarAdapter(
				getContext(), this);

		viewPager.setAdapter(adapter);
		viewPager.setVisibility(View.INVISIBLE);

		final TitlePageIndicator indicator = new TitlePageIndicator(
				getContext());
		if (mTypeface != null) {
		...
		viewPager.postDelayed(new Runnable() {
			...
			public void run() {
				...
				final int years = currentday.get(Calendar.YEAR)
						- getFirstValidDay().get(Calendar.YEAR);
				final int months = currentday.get(Calendar.MONTH)
						- getFirstValidDay().get(Calendar.MONTH);
				final int diffMonths = (years * 12) + months;
				viewPager.setCurrentItem(diffMonths, false);
	...
	public TextView getTextViewForDate(final long dayInMillis) {
		...
		for (int i = 0; i < childCount; i++) {
			...
			if (child != null && child instanceof ViewGroup) {
				...
				final CalendarView monthView = (CalendarView) ((ViewGroup) child)
						.getChildAt(0);
				if (monthView != null) {
					// Let the (single) CalendarView find a suitable TextView
					final TextView result = monthView
							.getTextViewForDate(dayInMillis);
					if (result != null) {
	...
	public void setOnTitlePageChangeListener(
			  ViewPager.OnPageChangeListener onPageChangeListener) {
		if ( mIndicator!= null) {
			mIndicator.setOnPageChangeListener(onPageChangeListener);
		}
		mOnTitlePageChangeListener = onPageChangeListener;
	}

	public void setOnCalendarLoadedListener(
			final OnCalendarLoadedListener listener) {
