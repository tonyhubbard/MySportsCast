...
public class CalendarView extends AbstractCalendarView implements
	...
	/**
	 * Will be called by the TextView This method will call the adapter's
	 * onDayClick method
	 */
	@Override
	...
	/**
	 * Create the view Initializes the headers and views for all visible days
	 */
	@Override
	protected void initView() {
		// Set the background color
		final int backgroundColor = DayStyleFactory
				.getBackgroundResourceForStyle(mDayStyle);
		...
		final int dayDisabledBackgroundColor = DayStyleFactory
				.getDayDisabledBackgroundColor(dayStyle, getResources());
		final int dayDisabledTextColor = getResources().getColor(
				R.color.lib_calendar_day_textcolor_disabled);
		final Typeface typeface = mTypeface;

		ViewGroup weekLayout = (ViewGroup) inflater.inflate(
				R.layout.lib_calendar_week, this, false);

		/*
		 * Continue adding days while: # We're adding the last few days of the
		 * previous month (modulo to work in january) # We're adding the days in
		 * the current month # We're adding the first few days of the next month
		 * (Add until we're at the end of the week)
		 */
		while ((currentDay.get(Calendar.MONTH) + 1) % MONTHS_IN_YEAR == currentMonth
			...
			if (lastDayOfWeek < firstDayOfWeek) {
				if ((dayOfWeek < firstDayOfWeek && dayOfWeek > lastDayOfWeek)
					...
					// we don't want to have this dayOfWeek in our calendar, so
					// move to the next one
					moveToNext = true;
				}
			} else if (dayOfWeek < firstDayOfWeek || dayOfWeek > lastDayOfWeek) {
				// we don't want to have this dayOfWeek in our calendar, so move
				// to the next one
				moveToNext = true;
			}
			if (moveToNext) {
			...
			final ViewGroup layout = DayStyleFactory.getDayLayoutForStyle(
					inflater, this, dayStyle);
			final TextView dayTextView = (TextView) layout
					.findViewById(R.id.lib_calendar_day_text);
			final ViewGroup categories = (ViewGroup) layout
					.findViewById(R.id.lib_calendar_day_categories);
			...
			/*
			 * We need to disable the view when: # The adapter says it should be
			 * disabled # This day is in another month (We fill the rows at the
			 * begin/end of the month) # This day is before the first valid day
			 * # This day is after the last valid day
			 */
			if ((adapter != null && !adapter.isDayEnabled(timeInMillis))
					|| (currentDay.get(Calendar.MONTH) != currentMonth)
					|| (firstValidDay != null && currentDay
							.before(firstValidDay))
				...
				if (adapter != null) {
					...
					final int[] colors = adapter
							.getCategoryColors(timeInMillis);
					if (colors != null) {
						for (final int color : colors) {
							// inflate a new category
							final View category = inflater.inflate(
									R.layout.lib_calendar_category, categories,
									false);

							// set the background color to the color provided by
							// the adapter
							category.setBackgroundColor(color);

							// add the view to the ViewGroup. Note that we can't
							// do this while inflating,
							// because that will cause the view to match the
							// size of his parent
			...
			// set tag to the timeInMillis for the onClickListener and to be
			// able to retrieve the TextView later on
			...
			if (dayOfWeek == lastDayOfWeek) {
				...
				weekLayout = (ViewGroup) inflater.inflate(
						R.layout.lib_calendar_week, this, false);
			}
	...
	/**
	 * Create the headers for each (visible) day of the week Starts at
	 * mFirstDayOfWeek, ends at mLastDayOfWeek
	 */
	private void createHeaders() {
		...
		final ViewGroup headers = (ViewGroup) inflater.inflate(
				R.layout.lib_calendar_headers, this, false);
		int dayOfWeek = firstDayOfWeek;

		do {
			// initialize variables for this day
			final TextView header = (TextView) inflater.inflate(
					R.layout.lib_calendar_single_header, headers, false);
			...
			for (int i = 0; i < childCount; i++) {
				...
				if (child instanceof ViewGroup) {
					...
					for (int index = 0; index < childItemCount; index++) {
						...
						if (dayView != null) {
							final ViewGroup.LayoutParams params = dayView
									.getLayoutParams();

							if (i == 0) {
								// This is the dayOfWeek TextView(header), so we
								// use wrap_content on the height
								params.width = dayWidth;
							} else {
								// This is the layout for a single day which is
								// a square
								params.width = dayWidth;
