...
public abstract class AbstractCalendarView extends LinearLayout {
	...
	private void parseAttributes(final Context context, final AttributeSet attrs) {
		...
		if (theme != null) {
			TypedArray a = theme.obtainStyledAttributes(attrs,
					R.styleable.AbstractCalendarView, 0, 0);

			try {
				// Retrieve the first day of week
				// noinspection ConstantConditions
				final int firstDay = a.getInt(
						R.styleable.AbstractCalendarView_firstDayOfWeek,
						Calendar.MONDAY);
				setFirstDayOfWeek(firstDay);

				// Retrieve the last day of week
				final int lastDay = a.getInt(
						R.styleable.AbstractCalendarView_firstDayOfWeek,
						Calendar.SUNDAY);
				setLastDayOfWeek(lastDay);

				// Retrieve the style for the day views
				mDayStyle = a.getInt(R.styleable.AbstractCalendarView_dayStyle,
						DayStyleFactory.DEFAULT_STYLE);
			} finally {
				// noinspection ConstantConditions
				a.recycle();
			}
	 ...
	 * @param dayStyle
	 *            The style
	 * @see com.vdesmet.lib.calendar.factory.DayStyleFactory DayStyleFactory:
	...
	public void setDayStyle(int dayStyle) {
		if (DayStyleFactory.isValidStyle(dayStyle)) {
			...
			throw new IllegalArgumentException(
					"Day Style is invalid. Check DayStyleFactory for options");
		}
	}

	/**
	 * Set the first day of the week that needs to be shown All days of the week
	 * are valid
	 * 
	 * @param day
	 *            Day of the week. e.g. Calendar.MONDAY
	 */
	public void setFirstDayOfWeek(final int day) {
		if (day < Calendar.SUNDAY || day > Calendar.SATURDAY) {
			throw new IllegalArgumentException("day must be between "
					+ Calendar.SUNDAY + " and " + Calendar.SATURDAY);
		}
	...
	/**
	 * (Optional) Set the last day of the week that needs to be shown All days
	 * of the week are valid If not set, it'll pick the day before the first day
	 * of the week Possible implementation: Hide the weekends ->
	 * setLastDayOfWeek(Calendar.FRIDAY)
	 * 
	 * @param day
	 *            Day of the week. e.g. Calendar.SUNDAY
	 */
	...
	/**
	 * (Optional) Set a custom last valid day. If set, all the days after this
	 * day will be disabled. If not set, this will be the last day of the month
	 * 
	 * @param lastValidDay
	 *            The last valid day
	 */
	...
	/**
	 * Set a DayAdapter The DayAdapter will be able to change the TextViews of
	 * the headers/days and is able to add Category Colors to days
	 * 
	 * @param newAdapter
	 *            The (new) adapter to be set
	 */
	 ...
	 * 
	 * @param newTypeFace
	 *            The new Typeface which will be used, or null for
	...
	protected void setDefaultTypeface() {
		...
		final Typeface defaultTypeface = Typeface.createFromAsset(
				getResources().getAssets(), "roboto_light.ttf");
		// setTypeface(defaultTypeface);
	}

	/**
	 * Set an onDayClick listener, which will be called when the user clicked on
	 * a valid Day
	 * 
	 * @param listener
	 *            Listener to respond to onClick events
	 */
	...
	/**
	 * Updates mFirstDayCalendar, so initView() knows on which day he needs to
	...
	private void updateCalendar() {
		...
		// get the number of days we need to remove from the calendar, to start
		// the calendar at mFirstDayOfWeek;
	...
	protected int getAvailableDayWidth(int width) {
		...
		final int paddingSides = getResources().getDimensionPixelSize(
				R.dimen.lib_calendar_day_padding_sides);
		final int screenWidth = width;
		final int daysInRow = getDaysInRow();
		final int availableWidth = screenWidth - (paddingSides * daysInRow * 2); // padding
																					// is
																					// at
																					// both
																					// sides(
		...
		final int maxWidthPerTile = getResources().getDimensionPixelSize(
				R.dimen.lib_calendar_day_size);

		// The maximum size of a tile(e.g. a single day)
		// This is either R.dimen.lib_calendar_day_size or the width which fits
		// the screen size
		return Math.min(widthPerTile, widthPerTile);
	}
	...
	protected void onMeasure(final int widthMeasureSpec,
		...
		if (resizeWidth) {
			...
			dayWidth = getResources().getDimensionPixelSize(
					R.dimen.lib_calendar_day_size);
		} else {
		...
		// Calculate our width, based on the width of a single day, the padding
		// and the number of days each week(row)
		...
		final TextView sampleHeader = (TextView) inflater.inflate(
				R.layout.lib_calendar_single_header, this, false);
		sampleHeader.setText(R.string.lib_header_monday);
		// Second, measure the TextView's height
		int textWidthMeasureSpec = View.MeasureSpec.makeMeasureSpec(
				measuredWidth / daysInRow, View.MeasureSpec.AT_MOST);
		int textHeightMeasureSpec = View.MeasureSpec.makeMeasureSpec(0,
		...
		if (this instanceof CalendarView) {
			...
			weeksInMonth = mFirstValidDay
					.getActualMaximum(Calendar.WEEK_OF_MONTH);
		} else {
			weeksInMonth = MAX_WEEKS_IN_MONTH;
		}
		final int weekHeight = dayWidth * weeksInMonth
				+ (paddingSides * (weeksInMonth + 1) * 2);

		// The height is the height of all the weeks, plus the headers
		measuredHeight = weekHeight + headerHeight;

		// If we're MultiCalendarView, also add the height of the
		// TitlePageIndicator
		if (this instanceof MultiCalendarView) {
			final MultiCalendarView multiCalendarView = (MultiCalendarView) this;
			final TitlePageIndicator indicator = multiCalendarView
					.getIndicator();

			// Use the width we measured, and let the Indicator calculate it's
			// height
			int indicatorWidthMeasureSpec = View.MeasureSpec.makeMeasureSpec(
					resolveSize(measuredWidth, widthMeasureSpec),
					MeasureSpec.EXACTLY);
			int indicatorHeightMeasureSpec = View.MeasureSpec.makeMeasureSpec(
					0, View.MeasureSpec.UNSPECIFIED);
			indicator.measure(indicatorWidthMeasureSpec,
					indicatorHeightMeasureSpec);
			final int indicatorHeight = indicator.getMeasuredHeight();

			// Now we have the height, update our ViewPager with the new
			// dimensions
			indicatorHeightMeasureSpec = View.MeasureSpec.makeMeasureSpec(
					measuredHeight, MeasureSpec.EXACTLY);
			final ViewPager viewPager = multiCalendarView.getViewPager();
			viewPager.measure(indicatorWidthMeasureSpec,
					indicatorHeightMeasureSpec);
	 ...
	 * @param resources
	 *            A resources object which can be retrieved by
	 *            Context.getResources()
	 * @return A name for this day of the week. MON - SUN.
	 * @throws IllegalArgumentException
	 *             Thrown when provided dayOfWeek is invalid
	 */
	protected String getNameForDay(final int dayOfWeek,
			final Resources resources) throws IllegalArgumentException {
		switch (dayOfWeek) {
		...
		default:
			// unknown day
			throw new IllegalArgumentException(
					"dayOfWeek is not valid. Pick a value between 1 and 7. "
							+ "dayOfWeek: " + dayOfWeek);
		}
	}
	...
	/**
	 * Get the number of days visible in one row. For example, from monday to
	 * friday -> 5
	 * 
