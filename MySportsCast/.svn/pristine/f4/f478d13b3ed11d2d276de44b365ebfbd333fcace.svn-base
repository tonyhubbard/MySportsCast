...
import com.mysportsshare.mysportscast.adapters.MyEventsAdapter;
...
import com.mysportsshare.mysportscast.utils.IntentUtils;
...
public class MyEventsFragment extends Fragment implements OnClickListener,
OnItemClickListener {
	...
	private String eventType = Constants.Event_filter_none;
	...
	public void updateUI() {
		...
		if(user_id.equalsIgnoreCase(SharedPreferencesUtils.getUserId())){
			...
			title.setText(user_name + " EVENTS");
		}
							...
							handler.postDelayed(new Runnable() {
								...
								public void run() {
									if (Utils.chkStatus()) {
										...
										if (reload) {
											servicetoFetchEvents(activity.getString(R.string.get_user_posted_events),
			...
			if (!Constants.longi.equals("0") && !Constants.lati.equals("0")) {
				if(eventsAdapter==null){
					...
					if(isCalendarEvent){
						...
						eventType = Constants.Event_filter_calendar;
						//TODO: fetch calendar events
						servicetoFetchEvents(getString(R.string.get_user_posted_events),
								Constants.longi,Constants.lati,"","","",
								String.valueOf(pageCount), eventType);
					}else{
	...
	private void showMenuitemDetails(int position) {
		...
		case 3:
			...
			i.putExtra(Constants.dataReceive,  Constants.data_cal_userPostEvents);
			startActivity(i);
			break;
	...
	public void setAdapter(List<EventInfo> eventsList, String pageId,
		...
		if (eventsAdapter == null && pageId.equalsIgnoreCase("0")) {
			eventsAdapter = new MyEventsAdapter(activity, eventsList,eventType,isfinishedevent);
			eventsListView.setAdapter(eventsAdapter);
		...
		public void onClick(View view) {
			...
			if (Utils.chkStatus()) {
				if (!Constants.longi.equals("0") && !Constants.lati.equals("0")) {
					...
					eventsAdapter = null;
					eventType = Constants.Event_filter_calendar;
					servicetoFetchEvents(getString(R.string.get_user_posted_cal_events),
							Constants.longi,Constants.lati,yearStr, monthStr,dayStr,
							String.valueOf(pageCount), eventType);
	...
	private void servicetoGetCalendarSummaryInfo() {
		...
		nameValuePair.add(new BasicNameValuePair(Constants.userProf_calendar_checked_Events,"0"));
	...
	/*
	 * Fetches user added events from the server 
	 */
	private void servicetoFetchEvents(final String myEvents,String longi, String lati,String yearStr, String monthStr, String dayStr,final String pageId, final String eventType){
		...
		String url = Constants.common_url + myEvents;


		List<NameValuePair> nameValuePairs = new ArrayList<NameValuePair>();
		nameValuePairs.add(new BasicNameValuePair(Constants.Events_userID, user_id));
		...
		if(yearStr.length()==0){
			...
			nameValuePairs.add(new BasicNameValuePair("user_check_in", "0"));
		}
		...
		JsonParser.callBackgroundService(url, nameValuePairs, new JsonServiceListener() {
			...
			@Override
			public void parseJsonResponse(String jsonResponse) {
				System.out.println(" Event Service RESPONSE: " + jsonResponse);
				if (jsonResponse!= null) {
					JSONObject jsonObject;
					try {
						jsonObject = new JSONObject(jsonResponse);
						...
						if (responseStr!=null && responseStr.equalsIgnoreCase(Constants.webserver_response_success)){
							...
							if(myEvents.equalsIgnoreCase(getString(R.string.get_user_posted_cal_events))){
								//calendar events
								upcomingEventsArr = resObj.getJSONArray(Constants.Events_Cal_InfoObj);
							}else{
								upcomingEventsArr = resObj.getJSONArray(Constants.Events_InfoObj);
							}
							if(upcomingEventsArr != null){
								for(int i = 0; i < upcomingEventsArr.length(); i++){
									...
									if(!TextUtils.isEmpty(upcomingEventsArr.getJSONObject(i).getString("latest_cast"))
											&& upcomingEventsArr.getJSONObject(i).getString("latest_cast").equalsIgnoreCase("no cast")){
						...
						String eventtype= "";
						if(myEvents.equalsIgnoreCase(getString(R.string.upcoming_events))){
							eventtype = "Attending";
							isfinishedevent = false;
						}else if(myEvents.equalsIgnoreCase(getString(R.string.inprogress_events))){
							eventtype = "here";
							isfinishedevent = false;
						}else if(myEvents.equalsIgnoreCase(getString(R.string.finished_events))){
							eventtype = "were here";
