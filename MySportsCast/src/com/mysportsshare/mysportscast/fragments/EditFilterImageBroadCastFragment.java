...
public class EditFilterImageBroadCastFragment extends Fragment implements
		OnClickListener {
	...
	int pos;
	static EditFilterImageBroadCastFragment f = new EditFilterImageBroadCastFragment();
	String taggedUsers;
	...
	public static Fragment getInstance(Context context) {
		if (f == null) {
			f = new EditFilterImageBroadCastFragment();
		}
		return f;
	}
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		if (args != null) {
			...
			if (args.getSerializable(Constants.KEY_PROFILE_EVENT_MEDIA) != null) {
				...
				profileEventMedia = (ProfileMediaInfo) args
						.getSerializable(Constants.KEY_PROFILE_EVENT_MEDIA);
				mediaId = profileEventMedia.getMediaId();
				Log.d("", "Koti edit filter image broadcast bundle if"+profileEventMedia.getMediaUrl());
				if (!TextUtils.isEmpty(profileEventMedia.getMediaUrl())) {
					BitmapUtils.setImages(profileEventMedia.getMediaUrl(),
							photo_iv, R.drawable.default_event_pic);
					photo_caption_et.setText(profileEventMedia
							.getMediaCaption());
					search_event_et.setText(profileEventMedia
							.getMediaEventTitle());
				}
			} else {
				Log.d("", "Koti edit filter image broadcast bundle else");
				imgPath = args.getString(Constants.dataReceive);
				mediaId = args.getString(Constants.KEY_EVENT_MEDIA_ID);
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		if (profileEventMedia == null) {
			if (TextUtils.isEmpty(imgPath)) {
				...
				BitmapUtils.setImages("file:///" + imgPath, photo_iv,
						R.drawable.profile_pic_dummy);
				twitterEventPic = "file:///" + imgPath;
				Bitmap bitmap = ((BitmapDrawable) photo_iv.getDrawable())
						.getBitmap();
				shareBitmap = bitmap;
	...
	public void onClick(View v) {
		switch (v.getId()) {
		case R.id.photo_iv:
			Log.d("","photo iv:"+mediaId);
			if(mediaId != null ){
				Fragment activeFragment = PhotosFragmentsFromHome
						.newInstance(activity);
				Bundle bundle = new Bundle();
				bundle.putString(Constants.KEY_EVENT_MEDIA_ID,
						mediaId);
		...
		case R.id.back_iv:
			...
			break;
		/*
		 * case R.id.search_people_llyt: if(getActivity()!=null){
		 * SearchCastPeopleFragment fragmentPeople = new
		 * SearchCastPeopleFragment();
		 * 
		 * Bundle peoplebundle = new Bundle();
		 * peoplebundle.putString("isfromHome", "photo"); if
		 * (!TextUtils.isEmpty(invitedUsers)) {
		 * peoplebundle.putParcelableArrayList("selected_users", (ArrayList<?
		 * extends Parcelable>) selectedInvitesUsersList); }
		 ...
		 * ((ImageFilterActivity)activity).push(fragmentPeople); }else
		 * if(activity instanceof MainActivity){
		 ...
		 * 
		 * if(getActivity()!=null){ if(!isFromHome){ SearchCastEvents1 fragment
		 * = new SearchCastEvents1(); Bundle bundle = new Bundle();
		 ...
		 * fragmentTransaction.commit(); if(activity instanceof
		 * ImageFilterActivity){ ((ImageFilterActivity)activity).push(fragment);
		 * }else if(activity instanceof MainActivity){ ((MainActivity)
		 * activity).pushFragments( ((MainActivity) activity).getmCurrentTab(),
		 * fragment, false, true); } } }
		 * 
		 * break;
		 */
	...
	private void displayOnActivity(Fragment activeFragment) {
		if (activity.getClass().getSimpleName()
			...
			((ImageFilterActivity)activity).popAllFragments();
	...
	private void serviceToUploadPhoto(final View view) {
		...
		String url = Constants.common_url + getString(R.string.update_photo);
					...
					try {
						if (jsonResponse != null) {
							...
							if (responseStr != null
								...
								Log.d("","Koti url:"+mediaUrl);
								if (mediaUrl == null||mediaUrl.equals("null")) {
									if(profileEventMedia != null){
										mediaUrl = profileEventMedia.getMediaUrl();
									}
	...
	private String uploadFile(String sourceUri, String ServerUrl) {
		...
		try {
			...
			if (getArguments().getBoolean(
					Constants.KEY_IS_EDIT_FILTER_IMAGE_FRAGMENT)) {
				Log.d("", "Koti upload edit filter image broadcast bundle if"
						+ mediaId);
				...
				Log.d("", "Koti upload edit filter image broadcast bundle else");
				entity.addPart("media_file", new StringBody(""));
				entity.addPart("media_id",
						new StringBody(profileEventMedia.getMediaId()));
